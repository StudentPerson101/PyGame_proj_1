...
try:
    right_click_weapon_image = pygame.image.load(right_click_weapon_image_path).convert_alpha()
except pygame.error as e:
...
# Load life image
try:
    life_image_original = pygame.image.load(life_image_path).convert_alpha()
...
# Load boss images
try:
    boss_image_original = pygame.image.load(boss_image_path).convert_alpha()
    boss_attack_image_original = pygame.image.load(boss_attack_image_path).convert_alpha()
except pygame.error as e:
...
running = True
game_over = False
game_state = 'menu'  # Possible states: 'menu', 'playing', 'rules', 'game_over', 'paused', 'victory'
...
while running:
    ...
    if game_state == 'menu':
        ...
        movement_text = font_small.render("Movement: W A S D", True, TEXT_COLOR)
        ...
        rules_title_rect = rules_title.get_rect(center=(SCREEN_WIDTH // 2, start_y))
        movement_rect = movement_text.get_rect(center=(SCREEN_WIDTH // 2, start_y + spacing * 1))
        aim_rect = aim_text.get_rect(center=(SCREEN_WIDTH // 2, start_y + spacing * 2))
        charged_attack_rect = charged_attack_text.get_rect(center=(SCREEN_WIDTH // 2, start_y + spacing * 3))
        auto_attack_rect = auto_attack_text.get_rect(center=(SCREEN_WIDTH // 2, start_y + spacing * 4))
        objective_rect = objective_text.get_rect(center=(SCREEN_WIDTH // 2, start_y + spacing * 5))
        back_text_rect = back_text.get_rect(center=(SCREEN_WIDTH // 2, start_y + spacing * 7))
        ...
        if not game_over:
            ...
            if score >= 200 and not boss_fight_active:
            ...
            for projectile in projectiles[:]:
                ...
                if projectile.is_off_screen():
                    ...
                    if boss_fight_active:
                        # Check for collision between projectiles and boss
                        ...
                        # Regular gameplay (before boss fight)
        ...
        # Draw asteroids if not in boss fight
        if not boss_fight_active:
            for asteroid in asteroids:
                asteroid.draw(screen)
        else:
            # Draw the boss and its projectiles
            boss.draw(screen)
