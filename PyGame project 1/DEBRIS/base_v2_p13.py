#  base - v2_p13 - player weaponry L2
...
initial_charge_radius = 150  # Starting radius for charging circle
...
while running:
    ...
    if game_state == 'menu':
        ...
        if is_charging:
            charge_duration = (current_time - charge_start_time) / 1000  # in seconds
            # Calculate the shrinking radius
            radius = initial_charge_radius * max(0, (max_charge_time - charge_duration) / max_charge_time)
            radius = max(radius, player1.size // 2 + 5)  # Ensure it doesn't shrink below player size
            if current_time - last_charge_flash_time >= charge_flash_interval:
                last_charge_flash_time = current_time
                charging_flash_visible = not charging_flash_visible
            if charging_flash_visible:
                # Create a surface just big enough to hold the circle
                circle_surface_size = int(radius * 2)
                ...
                # Draw the circle onto the circle_surface
                pygame.draw.circle(circle_surface, (255, 255, 255, 180), (int(radius), int(radius)), int(radius))
                # Blit the circle_surface onto the main screen at the correct position
                screen.blit(circle_surface, (int(player1.pos[0] - radius), int(player1.pos[1] - radius)))

        # Now draw the asteroids, projectiles, and player
        for asteroid in asteroids:
